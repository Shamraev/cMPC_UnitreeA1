...
>>>>>>> Stashed changes
    ...
    self.last_foot_contact_states = np.array(
        [True, True, True, True],
        dtype=np.int32)
    self.last_z = self._robot._base_position[2]
  ...
  def SLIP_traj(self, contacts, t):
    kp = 10
    kd = 2
    z = self._estimate_robot_height(contacts) # self._robot._base_position[2]
    z_des = self._desired_body_height

    contactStart = False
    offset = 0
    for leg_id in range(4):
      if self.last_foot_contact_states[leg_id]!=contacts[leg_id] and contacts[leg_id]: # start of contact
        contactStart = True
        offset = -((2*np.pi)/0.6)*t + np.pi

    #z_des = self._desired_body_height + 0.04*np.sin(((2*np.pi)/0.6)*t+offset)
    #z_des = z_des + kp*(self._desired_body_height-z)+kd*(0) # kp(z_des-z)+kd(vz_des-vz) 
    self.last_z = z
    return np.array((0., 0., z_des), dtype=np.float64)
  ...
  def get_action(self):
    ...
    desired_com_position = self.SLIP_traj(foot_contact_state, self._robot.GetTimeSinceReset())

    # [DEFAULT]
    robot_com_position = [0] # in _cpp_mpc.compute_contact_forces() robot_com_position will be calculated (noly z) if it wasn't set. 
    # [DEFAULT]
    # [TEST]
    contacts = np.array(
        [(leg_state in (gait_generator_lib.LegState.STANCE,
    ...
    # [TEST]   
    # [TEST2]
    robot_com_position = self._robot._base_position
    # [TEST2] 
